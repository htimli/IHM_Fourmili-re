...
public class Fourmiliere {
  ...
  public void initTest(){
    ...
    int decis=(int) (Math.random()* 100) % 3;  
    for(int i=0;i<=this.hauteur;i++){
        for(int j=0; j<=this.largeur; j++){
            switch(decis){
                case 0: this.ajouteFourmi(i, j); break;
                    
                case 1: this.setMur(i, j,true ); break;
                case 2: this.setQteGraines(i, j, 2); break;
                default: System.out.println("default"); break;
            }
            decis=(int) (Math.random()* 100) % 3;
            System.out.println(i+" j "+j + " decise:"+decis);
        }
    }
  }
